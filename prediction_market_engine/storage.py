...
class Storage:
    ...
    def get_historical_probabilities(
        ...
        for row in rows:
            ...
            result.append((ts, row["probability"]))
        return result
    ...
    def max_historical_gap(
        ...
        if not rows_a or not rows_b:
            return None
        by_time_a = {row["observed_at"]: row["probability"] for row in rows_a}
        gaps = []
        for row in rows_b:
            if row["observed_at"] in by_time_a:
                gaps.append(abs(by_time_a[row["observed_at"]] - row["probability"]) * 100)
    ...
    def get_opportunities(
        ...
        signals = self.get_signals(min_score=min_score, limit=500, active_only=True)
        ...
        for s in signals:
            ...
            opp = Opportunity(
                **s.model_dump(),
                ...
                min_volume=min(vol_a, vol_b) if vol_a and vol_b else max(vol_a, vol_b),
            )
            results.append(opp)
